...
function App() {
  ...
  async function runBacktest(endpoint) {
    ...
    try {
      ...
      const merged = (j && typeof j === 'object' && j.diagnostics && typeof j.diagnostics === 'object') ? { ...j, ...j.diagnostics } : j;
      setBtResult(merged);
      await fetchBaseline();
    } catch (e) {
      setBtError(String(e));
  ...
  <details>
    ...
    <div style={{ marginTop: 8, fontSize: 12, color: "#111827" }}>
      ...
      <div style={{ marginTop: 4 }}>
        Small spreads (line ≤ 3): {btResult.bucket_mae?.spread?.small == null ? "—" : btResult.bucket_mae.spread.small.toFixed(2)}
      </div>
      <div>
        Large spreads (line &gt; 3): {btResult.bucket_mae?.spread?.large == null ? "—" : btResult.bucket_mae.spread.large.toFixed(2)}
      </div>
      <div style={{ marginTop: 10 }}><strong>Total MAE</strong></div>
      <div style={{ marginTop: 4 }}>
        Low totals (&lt; 45): {btResult.bucket_mae?.total?.low == null ? "—" : btResult.bucket_mae.total.low.toFixed(2)}
      </div>
      <div>
        High totals (&gt; 60): {btResult.bucket_mae?.total?.high == null ? "—" : btResult.bucket_mae.total.high.toFixed(2)}
      </div>
    </div>
  </details>
