...
export default function Home(){
  ...
  const [division,setDivision]=useState<'fbs'|'fcs'|'ii'|'iii'>('fbs');
  ...
  return (<main className="min-h-screen p-6 flex flex-col gap-6 bg-gray-50">
    ...
    <section className="bg-white rounded-2xl shadow p-4"><div className="flex flex-wrap items-center gap-4">
      <div className="flex items-center gap-2"><span className="text-sm">Division</span><select value={division} onChange={e=>setDivision(e.target.value as any)} className="border rounded p-1"><option value="fbs">FBS</option><option value="fcs">FCS</option><option value="ii">II</option><option value="iii">III</option></select></div>
    ...
    {out && (<section className="bg-white rounded-2xl shadow p-4 overflow-x-auto"><h2 className="font-semibold mb-2">Predictions</h2><table className="min-w-full text-sm"><thead><tr className="text-left border-b"><th className="py-1 pr-4">Home</th><th className="py-1 pr-4">Away</th><th className="py-1 pr-4">Neutral</th><th className="py-1 pr-4">Elo %</th><th className="py-1 pr-4">Stats %</th><th className="py-1 pr-4">Market %</th><th className="py-1 pr-4">Final %</th><th className="py-1 pr-4">Spread</th><th className="py-1 pr-4">Favorite</th></tr></thead><tbody>{out.predictions?.map((p:any,i:number)=>(<tr key={i} className="border-b"><td className="py-1 pr-4">{p.home_team}</td><td className="py-1 pr-4">{p.away_team}</td><td className="py-1 pr-4">{p.neutral_site?'Yes':'No'}</td><td className="py-1 pr-4">{(p.p_elo*100).toFixed(1)}%</td><td className="py-1 pr-4">{(p.p_stats*100).toFixed(1)}%</td><td className="py-1 pr-4">{p.p_market!=null?(p.p_market*100).toFixed(1)+'%':'—'}</td><td className="py-1 pr-4 font-medium">{(p.home_win_prob*100).toFixed(1)}%</td><td className="py-1 pr-4">{p.proj_spread.toFixed(1)}</td><td className="py-1 pr-4 font-medium">{p.favorite}</td></tr>))}</tbody></table><p className="text-xs text-gray-500 mt-2">Tuned HFA: {out.tuned_hfa?.toFixed?.(2)}</p></section>)}
