...
def _q(sorted_samples: list[float], q: float) -> float:
    """Quantile helper for already-sorted numeric samples.

    Uses linear interpolation between nearest ranks.

    Robustness rules (to avoid serverless 500s):
    - empty input -> 0.0
    ...
    hi = min(lo + 1, n - 1)
    frac = pos - lo
    a = float(sorted_samples[lo])
    b = float(sorted_samples[hi])
    return a + (b - a) * frac

def _team_power_from_ratings(r: dict) -> float:
    """Compute a simple power proxy from component ratings.
    Higher = stronger. Uses only model ratings (no market leakage).
    """
    off = (float(r.get("off_rush", 50.0)) + float(r.get("off_pass", 50.0))) / 2.0
    deff = (float(r.get("def_rush", 50.0)) + float(r.get("def_pass", 50.0))) / 2.0
    st = float(r.get("st", 0.0))
    return off - deff + st


def _prior_spread_from_gap(gap: float) -> float:
    """Convert power gap into a conservative prior spread (home - away)."""
    # 0.6 pts of spread per 1 pt of power gap, capped to avoid runaway blowouts.
    prior = gap * 0.6
    if prior > 28.0:
        return 28.0
    if prior < -28.0:
...
def _run_backtest(
    ...
    with SessionLocal() as sess:
        ...
        for g, ht, at in games:
            ...
            # Tier-aware prior spread blending for extreme mismatches
            try:
                gap = _team_power_from_ratings(home_r) - _team_power_from_ratings(away_r)
            except Exception:
                gap = 0.0
            if abs(gap) >= 18.0:
                prior_spread = _prior_spread_from_gap(gap)
                # Blend: keep the sim signal, but inject a prior so FBS/FCS-type mismatches aren't compressed.
                model_spread_home = (0.7 * model_spread_home) + (0.3 * prior_spread)
...
def simulate_series_by_name_auto(req: NamesAutoSeriesIn):
    ...
    resp = {
        ...
        "expected_spread": ( (mean_home - mean_away) if abs(_team_power_from_ratings(home_r) - _team_power_from_ratings(away_r)) < 18.0 else (0.7 * (mean_home - mean_away) + 0.3 * _prior_spread_from_gap(_team_power_from_ratings(home_r) - _team_power_from_ratings(away_r))) ),
        "stdev_score_home": (
...
def backtest_get_baseline(test_season: int = 2023) -> dict:
    ...
    Response shape is consistent with other backtest endpoints:
      { ok, baseline, candidate, verdict, delta, diagnostics, meta }
    """
    with SessionLocal() as sess:
        row = (
            sess.execute(
        ...
        if not row:
            return {
                ...
                "delta": None,
                "diagnostics": None,
                "meta": {"test_season": int(test_season)},
            }
...
def backtest_set_baseline(req: BacktestIn) -> dict:
    ...
    baseline_metrics.pop("diagnostics", None)
    ...
    return {
        ...
        "delta": None,
        "diagnostics": diag,
        "meta": {"test_season": int(req.test_season), "n_games": int(req.n_games), "n_sims": int(req.n_sims)},
    }
...
def backtest_run(req: BacktestIn) -> dict:
    ...
    Returns verdict PASS/EVEN/FAIL relative to the stored baseline for that season.
    Response shape is consistent across backtest endpoints:
      { ok, baseline, candidate, verdict, delta, diagnostics, meta }
    """
    try:
        metrics = _run_backtest(
        ...
        with SessionLocal() as sess:
            ...
            if not baseline:
                raise HTTPException(status_code=400, detail=f"No baseline found for season {int(req.test_season)}. Click 'Set baseline' first.")

            # Materialize baseline fields while the Session is open.
            # If we access ORM attributes after exiting this context, SQLAlchemy can raise
            # DetachedInstanceError (common in serverless where sessions are short-lived).
            baseline_metrics = {
                "id": int(baseline.id) if baseline.id is not None else None,
                "created_at": baseline.created_at,
                "label": baseline.label,
                "test_season": int(baseline.test_season) if baseline.test_season is not None else None,
                "n_games": int(baseline.n_games) if baseline.n_games is not None else None,
                "n_sims": int(baseline.n_sims) if baseline.n_sims is not None else None,
                "spread_mae": float(baseline.spread_mae) if baseline.spread_mae is not None else None,
                "total_mae": float(baseline.total_mae) if baseline.total_mae is not None else None,
                "brier": float(baseline.brier) if baseline.brier is not None else None,
            }
            ...
            sess.add(
                ModelEval(
                    ...
                    n_games=int(metrics["n_games"]),
                    n_sims=int(metrics["n_sims"]),
                    spread_mae=float(metrics["spread_mae"]) if metrics.get("spread_mae") is not None else None,
                    total_mae=float(metrics["total_mae"]) if metrics.get("total_mae") is not None else None,
                    brier=float(metrics["brier"]) if metrics.get("brier") is not None else None,
                )
            )
            sess.commit()

        # NOTE: baseline_metrics was materialized inside the Session block above.

        # Candidate metrics & diagnostics.
        diag = metrics.get("diagnostics")
        candidate_metrics = dict(metrics)
        candidate_metrics.pop("diagnostics", None)

        # Deltas are baseline - candidate (positive means candidate is better).
        def _d(a, b):
        ...
        # IMPORTANT: do not reference ORM objects outside the Session context.
        # Use the materialized baseline_metrics dict created inside the Session block.
        delta_spread = _d(baseline_metrics.get("spread_mae"), candidate_metrics.get("spread_mae"))
        delta_total = _d(baseline_metrics.get("total_mae"), candidate_metrics.get("total_mae"))
        delta_brier = _d(baseline_metrics.get("brier"), candidate_metrics.get("brier"))

        # Verdict logic: pass if improves at least one metric and does not materially regress others.
        # Material threshold = 1% relative or absolute tiny guard.
        def _improved(delta):
            return delta is not None and delta > 0

        def _regressed(delta):
            return delta is not None and delta < 0

        improved = any([_improved(delta_spread), _improved(delta_total), _improved(delta_brier)])
        regressed = any([_regressed(delta_spread), _regressed(delta_total), _regressed(delta_brier)])

        if improved and not regressed:
            verdict = "PASS"
            reason = "Candidate improves at least one primary metric with no regressions."
        elif regressed and not improved:
            verdict = "FAIL"
            reason = "Candidate regresses at least one primary metric with no improvements."
        else:
            verdict = "EVEN"
            reason = "Mixed or within noise."
        ...
        return {
            ...
            "diagnostics": diag,
            "meta": {"test_season": int(req.test_season), "n_games": int(req.n_games), "n_sims": int(req.n_sims), "reason": reason},
        }

    except Exception as e:
        import traceback
        tb = traceback.format_exc()
        print('BACKTEST_RUN_ERROR', tb)
        raise HTTPException(status_code=500, detail={'error': str(e), 'trace': tb[:4000]})